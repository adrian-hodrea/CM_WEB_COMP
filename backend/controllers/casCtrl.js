const casDBApi = require('../db_apis/casDBApi.js');
...
async function insert(bodyData, res) {
  ...
  try {
    const result = await casDBApi.insert(bodyData);
    insertMsg = {
      "message": "Casa de Sanatate a fost adaugata cu succes in baza de date"
    }
    ...
    if (err.errno == 1062) {
      insertMsg = {
        "message": "Casa de sanatate cu acest CUI exista deja in sistem"
      }
...
async function select(req,res) {
  ...
  try {
    const caseDeSanatate = await casDBApi.select();
    res.status(200).json(caseDeSanatate)
  }
  catch (err) {
    selectMsg = {
...
async function selectChooseList(req,res) {
  ...
  try {
    const casChooseList = await casDBApi.selectChooseList();
    res.status(200).json(casChooseList)
  }
  catch (err) {
    selectMsg = {
      "message": "A aparut o problema, datele nu pot fi afisate"
